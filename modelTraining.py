...
class SFCN(nn.Module):
    def __init__(self, channel_number=[32, 64, 128, 256, 256, 64], output_dim=1, dropout=True):
        ...
        avg_shape = [5, 6, 5]  # Average pooling size
        ...
        self.classifier.add_module('conv_final', nn.Conv3d(in_channel, out_channel, padding=0, kernel_size=1))
    ...
    def forward(self, x):
        ...
        x = self.classifier(x)
        x = x.view(x.size(0), -1)  # Flatten output for MSE loss
        return x
...
def train_model(epochs, model_save_path="sfcn_brain_age_model.pth"):
    ...
    for epoch in range(epochs):
        ...
        epoch_model_path = f"epoch_{epoch + 1}_sfcn_model.pth"
...
# Evaluate the model
def evaluate_model(model_load_path="sfcn_brain_age_model.pth"):
    ...
    results_df.to_csv("sfcn_test_results.csv", index=False)
    print("Results saved to sfcn_test_results.csv")
